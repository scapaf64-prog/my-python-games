...
try:
    ...
    stop_sound = None
# try:
#     coin_sound = pygame.mixer.Sound(os.path.join(BASE, "muenze1.wav"))
...
FLASH_SPEED = 20        # 2 gleiche Steine
JACKPOT_SPEED = 18      # 3 gleiche Steine
...
class SlotWheel:
    def __init__(self, x, y):
        ...
        # Startanzeige mit mehreren Steinen
    ...
    def update(self, dt):
        ...
        # kleines Nachfedern am Ende
        self.bounce = 0
        if self.t > 0.92:
            p = (self.t - 0.92) / 0.08      # 0 ... 1
    ...
    def draw(self, screen):
        ...
        # Walzen oben und unten gezeichnet
        ...
        # unten (gespiegelt)
        screen.blit(
            pygame.transform.flip(fade, False, True),
            (self.x, self.y + WINDOW_SIZE - 22)
        )
...
class SlotMachine:
    def __init__(self):
        ...
        self.bet = 1
        self.win = 0
        self.win_cooldown = 0        # nach Gewinn mind. 3 Nieten, nach Jackpot mind. 5 Nieten
        self.cheat_spin = False      # außer nach "z" und "d" (cheat-tasten)

        # Auszahlung
        self.pay_speed = 10          # Coins pro Sekunde
        self.pay_timer = 0.0
    ...
    def lerp_color(self, c1, c2, t):       # Lineare Interpolation
        return (
    ...
    def is_win(self, a, b, c):             # Funktion liefert nur True or False
    ...
    def check_win(self):
        ...
        # Cheats dürfen den normalen Spielverlauf nicht beeinflussen
...
#                 coin_sound.play()
    ...
    def spin(self, cheat=None):
        ...
        if cheat == "THREE":      # "d" löst Jackpot aus ("dreier")
            a = random.randint(1,7)
            b = a
            c = a

        elif cheat == "TWO":      # "z" löst Dobble aus ("zweier")
            a = random.randint(1,7)
            b = a
            ...
            # zufällig bestimmen, welche zwei gleich sind
            ...
            if self.win_cooldown > 0:
                ...
                # solange ziehen, bis KEIN Gewinn entsteht
    ...
    def draw_message(self, screen, small_font, big_font):
       ...
       if self.state == FLASH:
           ...
           # einmaliges sanftes Aufleuchten
           if t < 1.8:
           ...
           # Transparenz synchron zum Puls
           if self.message_time < 2.0:
...
def draw_display(screen, label_font, display_font,
                     title, value, x, y, width):
...
def draw_reel_background(screen, rect, base_color):
    ...
    for y in range(rect.height):
        ...
        # Abstand von der Mitte (0 ... 1)
        d = abs((y / rect.height) - 0.5) * 2

        # Mitte heller
#         brightness = 1.35 - d * 0.45
        brightness = 1.60 - d * 0.60     # noch hellere MItte
