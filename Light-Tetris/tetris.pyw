...
class GlowCube:
    ...
    def update(self, center_x, center_y):
        ...
        self.current_cx += (target_cx - self.current_cx) * self.lerp_speed
        self.current_cy += (target_cy - self.current_cy) * self.lerp_speed

    def draw(self, screen, is_next=False):
        draw_x = self.current_cx - self.size / 2
        draw_y = self.current_cy - self.size / 2

        # DESIGN-ANPASSUNG: Wenn es das next_piece ist, leuchtet es abgedunkelt (wie liegende Steine)
        if is_next:
            ...
            # Unverändertes Original-Leuchten für aktive Steine
...
class Tetromino:
    ...
    # DESIGN-ANPASSUNG: Akzeptiert nun das Flag und reicht es an die Würfel weiter
