...
class Tetris:
    ...
    def clear_lines(self):
        ...
        if self.lines_to_clear:
            ...
            target_id = SHAPES.index(self.next_piece['shape']) + 1
            self.wheel.start(target_id)
            self.wheel.spin_speed = 12  # falls vorhanden
            self.wheel.t = 0            # reset animation

            self.current_piece["x"] = GRID_WIDTH // 2 - len(self.current_piece["shape"][0]) // 2
            self.current_piece["y"] = 0

            # WICHTIG: echter Spawn-Check
    ...
    def update_animation(self, dt):
        ...
        # zuerst neue Pieces vorbereiten
        self.current_piece = self.next_piece
        self.next_piece = self.new_piece()
        ...
        # IMPORTANT: State erst danach setzen
        self.state = "PLAY"
